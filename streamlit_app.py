...
# APP
    ...
    with tab_ronda:
        ...
        try:
            courses = supabase.table("courses").select("*").execute().data or []
        except Exception:
            courses = []
        course_options = {c["name"]: c["id"] for c in courses}

        st.selectbox("Selecciona el Campo", list(course_options.keys()), index=None, placeholder="Selecciona un campo...", key="ronda_course")
        ...
        st.selectbox("Selecciona las Tees", _tk, index=None if tee_options else 0, placeholder="Selecciona las tees...", key="ronda_tee")

        try:
            _pl = supabase.table("players").select("*").order("name").execute().data or []
        except Exception:
            _pl = []
        player_options = {p["name"]: p["id"] for p in _pl}
        ...
        front_scores = st.data_editor(pd.DataFrame({"Hoyo": list(range(1,10)), "Score": [0]*9}}), hide_index=True, use_container_width=True, key={q}ronda_front{q})
        st.subheader("Back 9")
        back_scores = st.data_editor(pd.DataFrame({"Hoyo": list(range(10,19)), "Score": [0]*9}}), hide_index=True, use_container_width=True, key={q}ronda_back{q})
        ...
        if st.button("Guardar Ronda", key="btn_guardar_ronda", use_container_width=True, type="primary"):
            _pname = st.session_state.get("ronda_player")
            _cname = st.session_state.get("ronda_course")
            _tname = st.session_state.get("ronda_tee")
            if not _pname or not _cname or not _tname:
                st.error("Selecciona campo, tees y jugador.")
            else:
                try:
                    _tee = tee_options.get(_tname)
                    if not _tee:
                        st.error("Tee no encontrado.")
                    else:
                        _pid  = player_options[_pname]
                        _cid2 = course_options[_cname]
                        _rid  = str(uuid.uuid4())
